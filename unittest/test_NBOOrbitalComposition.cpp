...
TEST(TestNBOOrbitalComposition, SpiritParseLine) {
    ...
    std::optional<bf::vector<int, std::string, std::string, double>> attribute
    ...
    ASSERT_THAT(bf::at_c<2>(*attribute), Eq("5s"));
    ASSERT_THAT(bf::at_c<3>(*attribute), DoubleEq(0.0));
}

TEST(TestNBOOrbitalComposition, SpiritParseLineFail) {
    namespace bf = boost::fusion;
    std::optional<bf::vector<int, std::string, std::string, double>> attribute
            = NBOOrbitalComposition::parseLine("    NAO#   Center   Label      Type      Composition");
