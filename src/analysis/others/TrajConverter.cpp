...
void TrajConverter::process() {
    ...
    while (!ifstream.eof() and std::getline(ifstream, line)) {
        if (boost::starts_with(line, "block")) {
            ...
            os << boost::format(fmt) % ++seq
                  % boost::fusion::at_c<0>(attribute)
                  % "" % 0
                  % (boost::fusion::at_c<1>(attribute) * bohr2Ang)
                  % (boost::fusion::at_c<2>(attribute) * bohr2Ang)
                  % (boost::fusion::at_c<3>(attribute) * bohr2Ang)
                  % boost::fusion::at_c<0>(attribute);
            if (total_atoms == seq) {
                seq = 0;
                os << "TER\nENDMDL\n";
