...
boost::optional<boost::fusion::vector<std::string, double, double>> ADCHCharge::read_charge(const std::string &line) {
    ...
    static const auto parser =
        copy("Atom:" >> omit[int_] >> lexeme[+alpha] >> "Corrected charge:" >> double_ >> "Before:" >> double_);
...
void ADCHCharge::process(const std::string &file, const std::vector<int> &atoms) {
    ...
    std::cout << std::setw(7) << "Average:" << std::setw(14) << boost::accumulators::mean(adch_acc) << std::setw(15)
              << boost::accumulators::mean(hirshfeld_acc) << '\n';
}
