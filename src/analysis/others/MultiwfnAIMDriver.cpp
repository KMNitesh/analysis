...
void MultiwfnAIMDriver::readBCP(std::istream &is, std::vector<BCP> &bcp_vector) {
    ...
    while (std::getline(is, line), !boost::contains(line, "============ Modify or export found CPs ============")) {
        ...
        if (auto it = std::begin(line);
            qi::phrase_parse(it, std::end(line), parser, ascii::space, attribute) and it == std::end(line)) {
...
std::vector<double MultiwfnAIMDriver::BCP_property::*> MultiwfnAIMDriver::parse_options(
...
void MultiwfnAIMDriver::process(const std::string &file, const std::vector<std::pair<int, int>> &bonds,
    ...
    for (auto &bcp : bcp_vector) {
        ...
        while (c.running() and std::getline(is, line) and
               !boost::contains(line, "================ Topology analysis ===============")) {
...
void MultiwfnAIMDriver::printProperty(const boost::format &fmt, const std::vector<BCP> &bcp_vector,
    ...
    boost::for_each(bcp_vector, [&](const BCP &bcp) {
        std::cout << boost::format(fmt) % bcp.atom_pair_belonging.first % bcp.atom_pair_belonging.second %
                         bcp.property.*field;
...
boost::bimap<unordered_set_of<double MultiwfnAIMDriver::BCP_property::*>, unordered_set_of<std::string>>
    MultiwfnAIMDriver::property_names = list_of<decltype(MultiwfnAIMDriver::property_names)::relation>(
...
boost::bimap<unordered_set_of<double MultiwfnAIMDriver::BCP_property::*>, unordered_set_of<std::string>>
    MultiwfnAIMDriver::property_options = list_of<decltype(MultiwfnAIMDriver::property_options)::relation>(
        ...
        &MultiwfnAIMDriver::BCP_property::Hb, "hb")(&MultiwfnAIMDriver::BCP_property::Ellipticity, "ell");

std::vector<double MultiwfnAIMDriver::BCP_property::*> MultiwfnAIMDriver::option_menu() {
    std::vector<double BCP_property::*> show_options{&BCP_property::Density_of_all_electrons,
                                                     &BCP_property::Laplacian_of_electron_density, &BCP_property::Hb,
    ...
    for (;;) {
        ...
        if (auto it = begin(line); qi::phrase_parse(it, end(line), +num__, ascii::space, options) and it == end(line)) {
            return options;
        }
...
std::pair<std::string, std::vector<std::pair<int, int>>> MultiwfnAIMDriver::user_input() {
    ...
    qi::rule<std::string::iterator, std::pair<int, int>(), qi::ascii::space_type> bond_parser =
        ('(' >> qi::int_ >> ',' >> qi::int_ >>
