...
void HBond::processFirstFrame(std::shared_ptr<Frame> &frame) {
    for (const auto &atom : frame->atom_list) {
        ...
        if (mode not_eq Selector::Both and Atom::is_match(atom, mask2) and donor_acceptor_symbols.contains(which(atom)))
            acceptors.push_back(atom);
    };
}
...
bool HBond::check_hbond(const std::shared_ptr<Atom> &donor, const std::shared_ptr<Atom> &hydrogen,
    ...
    if (atom_distance(donor, acceptor, frame) <= donor_acceptor_dist_cutoff) {
        ...
        if (std::abs(radian * std::acos(cosine)) <= angle_cutoff) {
            record_hbond(hydrogen, acceptor, frame);
            return true;
        }
    }
    return false;
...
void HBond::record_hbond(const std::shared_ptr<Atom> &hydrogen, const std::shared_ptr<Atom> &acceptor,
                         const std::shared_ptr<Frame> &frame) {

    hbdist.back()[std::array{hydrogen, acceptor}] = atom_distance(hydrogen, acceptor, frame);
}

void HBond::output_distance_statistics(std::ostream &os) {
    std::map<std::array<std::shared_ptr<Atom>, 2>, std::size_t> countor_map;
    for (auto &seq : hbdist) {
        for (auto &[key, value] : seq) {
            ++countor_map[key];
        }
    }
    std::vector<std::pair<std::array<std::shared_ptr<Atom>, 2>, std::size_t>> countor(begin(countor_map),
                                                                                      end(countor_map));
    ...
    for (const auto &p : countor) {
        const auto &hydrogen = p.first[0];
        os << std::setw(15) << (boost::format(":%d@%s") % hydrogen->residue_num.value() % hydrogen->atom_name).str();
    }
    os << '\n';

    os << "#      ";
    for (const auto &p : countor) {
        const auto &donor = p.first[1];
        os << std::setw(15) << (boost::format(":%d@%s") % donor->residue_num.value() % donor->atom_name).str();
    }
    os << '\n';
