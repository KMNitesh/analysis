...
namespace {
template <typename Function> class RotAcfParallelBody {
public:
    const std::vector<std::vector<std::tuple<double, double, double>>> &rots;
    std::vector<double, tbb::tbb_allocator<double>> acf;

    size_t array_length;
    size_t max_time_gap_step;
    Function f;

    explicit RotAcfParallelBody(const std::vector<std::vector<std::tuple<double, double, double>>> &rots,
                                size_t max_time_gap_step, size_t array_length, Function f)
        : rots(rots), acf(array_length), array_length(array_length), max_time_gap_step(max_time_gap_step),
          f(std::move(f)) {}

    RotAcfParallelBody(const RotAcfParallelBody &rhs, tbb::split)
        : rots(rhs.rots), acf(rhs.array_length), array_length(rhs.array_length),
          max_time_gap_step(rhs.max_time_gap_step), f(rhs.f) {}
    ...
    void operator()(const tbb::blocked_range<std::size_t> &range) {
        for (auto index = range.begin(); index != range.end(); index++) {
            ...
            for (size_t i = 0; i < total_size - 1; i++) {
                for (size_t j = i + 1; j < std::min(total_size, max_time_gap_step + i); j++) {
                    auto m = j - i;
...
template <typename Function> std::vector<double> RotAcf::calculate(Function f) const {
    size_t max_time_gap_step = std::ceil(max_time_gap / time_increment_ps) + 1;

    auto array_length = std::min(rots[0].size(), max_time_gap_step);
    RotAcfParallelBody parallelBody(rots, max_time_gap_step, array_length, f);
...
void RotAcf::setParameters(const std::shared_ptr<VectorSelector> &vector, int LegendrePolynomial,
    ...
    if (!(LegendrePolynomial >= 1 and LegendrePolynomial <= static_cast<int>(LegendreStr.size()))) {
    ...
    if (max_time_gap_ps <= 0) {
        throw std::runtime_error("`max_time_gap_ps` must be postive");
    } else if (max_time_gap_ps <= this->time_increment_ps) {
        throw std::runtime_error("`max_time_gap_ps` must be larger than `time_increment_ps`");
    } else {
