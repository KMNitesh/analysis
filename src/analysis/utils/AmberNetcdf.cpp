...
static int GetDimInfo(int ncid, const char *attribute, int *length) {
    ...
    *length = 0;
    ...
    // Get dimid
    if (checkNCerr(nc_inq_dimid(ncid, attribute, &dimID), "ncGetDimInfo: Getting dimID for attribute %s", attribute) !=
        0)
        return -1;

    // get Dim length
    if (checkNCerr(nc_inq_dimlen(ncid, dimID, &slength), "ncGetDimInfo: Getting length for attribute %s", attribute) !=
        0)
        return -1;
...
static char *GetAttrText(int ncid, int vid, const char *attribute) {
    ...
    char *attrText;
    // Get attr length
    if (checkNCerr(nc_inq_attlen(ncid, vid, attribute, &attlen), "ncGetAttrText: Getting length for attribute %s\n",
                   attribute))
        return NULL;
    // Allocate space for attr text, plus one for NULL char
    attrText = (char *)malloc((attlen + 1) * sizeof(char));
    // Get attr text
    if (checkNCerr(nc_get_att_text(ncid, vid, attribute, attrText), "ncGetAttrText: Getting attribute text for %s\n",
                   attribute)) {
        free(attrText);
        return NULL;
    }
 ...
 */
int netcdfLoad(struct AmberNetcdf *A, const char *filename) {
    int debug;
    char *attrText;
    ...
    // Get coordinate info
    if (checkNCerr(nc_inq_varid(A->ncid, NCCOORDS, &(A->coordVID)), "Getting coordinate VID") != 0) return 1;
    // Get spatial info
    ...
    if (A->isNCrestart) {
        ...
        if (nc_inq_varid(A->ncid, NCTIME, &(A->timeVID)) == NC_NOERR) {
            ...
            if (checkNCerr(nc_get_var_double(A->ncid, A->timeVID, &(A->restartTime)), "Getting netcdf restart time."))
                return 1;
    ...
    if (nc_inq_varid(A->ncid, NCCELL_LENGTHS, &(A->cellLengthVID)) == NC_NOERR) {
        if (checkNCerr(nc_inq_varid(A->ncid, NCCELL_ANGLES, &(A->cellAngleVID)), "Getting cell angles.") != 0) return 1;
...
int netcdfOpen(struct AmberNetcdf *A, const char *filename) {
    // Open for write
    int debug;
    char *attrText;
    ...
    // Do not print error message on error, just return 1
    // This allows one to gracefully check if this is a netcdf file
    if (nc_open(filename, NC_WRITE, &(A->ncid)) != NC_NOERR) return 1;
...
int netcdfClose(struct AmberNetcdf *A) {
    if (A == NULL) return 0;
    if (A->ncid != -1) checkNCerr(nc_close(A->ncid), "Closing netcdf file.");
 ...
 */
int netcdfWriteRestart(char *filename, int natom, double *X, double *Velo, double *box, double restart_time,
                       double restart_temp) {
    ...
    char xyz[3];
    double restartTime;
    char abc[15] = {'a', 'l', 'p', 'h', 'a', 'b', 'e', 't', 'a', ' ', 'g', 'a', 'm', 'm', 'a'};
    char *title;
    ...
    // Create file
    if (checkNCerr(nc_create(filename, NC_64BIT_OFFSET, &(A.ncid)), "Creating Netcdf restart file %s", filename))
        return 1;
    // if (debug>0)
    printf("    Successfully created Netcdf restart file %s, ncid %i\n", filename, A.ncid);

    // Time variable
    if (checkNCerr(nc_def_var(A.ncid, NCTIME, NC_DOUBLE, 0, dimensionID, &(A.timeVID)), "Defining time variable."))
        return 1;
    if (checkNCerr(nc_put_att_text(A.ncid, A.timeVID, "units", 10, "picosecond"), "Writing time VID units.")) return 1;

    // Spatial dimension and variable
    if (checkNCerr(nc_def_dim(A.ncid, NCSPATIAL, 3, &(A.spatialDID)), "Defining spatial dimension.")) return 1;
    ...
    A.ncatom = natom;
    A.ncatom3 = natom * 3;
    if (checkNCerr(nc_def_dim(A.ncid, NCATOM, A.ncatom, &(A.atomDID)), "Defining atom dimension.")) return 1;
    ...
    if (checkNCerr(nc_def_var(A.ncid, NCCOORDS, NC_DOUBLE, 2, dimensionID, &(A.coordVID)),
                   "Defining coordinates variable."))
        return 1;
    if (checkNCerr(nc_put_att_text(A.ncid, A.coordVID, "units", 8, "angstrom"), "Writing coordinates variable units."))
        return 1;
    ...
    if (Velo != NULL) {
        ...
        if (checkNCerr(nc_put_att_text(A.ncid, A.velocityVID, "units", 19, "angstrom/picosecond"),
            ...
            return 1;
        if (checkNCerr(nc_put_att_double(A.ncid, A.velocityVID, "scale_factor", NC_DOUBLE, 1, &velocityScale),
                       "Writing velocities scale factor."))
            return 1;
    }

    // Cell Spatial
    if (checkNCerr(nc_def_dim(A.ncid, NCCELL_SPATIAL, 3, &(A.cell_spatialDID)), "Defining cell spatial dimension."))
        return 1;
    dimensionID[0] = A.cell_spatialDID;
    if (checkNCerr(nc_def_var(A.ncid, NCCELL_SPATIAL, NC_CHAR, 1, dimensionID, &(A.cell_spatialVID)),
                   "Defining cell spatial variable."))
        return 1;

    // Cell angular
    if (checkNCerr(nc_def_dim(A.ncid, NCLABEL, NCLABELLEN, &(A.labelDID)), "Defining label dimension.")) return 1;
    if (checkNCerr(nc_def_dim(A.ncid, NCCELL_ANGULAR, 3, &(A.cell_angularDID)), "Defining cell angular dimension."))
        return 1;
    ...
    if (checkNCerr(nc_def_var(A.ncid, NCCELL_ANGULAR, NC_CHAR, 2, dimensionID, &(A.cell_angularVID)),
                   "Defining cell angular variable."))
        return 1;
    ...
    // Attributes
    if (checkNCerr(nc_put_att_text(A.ncid, NC_GLOBAL, "title", strlen(title), title), "Writing title.")) return 1;
    if (title != NULL) free(title);
    if (checkNCerr(nc_put_att_text(A.ncid, NC_GLOBAL, "application", 5, "AMBER"), "Writing application.")) return 1;
    if (checkNCerr(nc_put_att_text(A.ncid, NC_GLOBAL, "program", 11, "AmberNetcdf"), "Writing program.")) return 1;
    if (checkNCerr(nc_put_att_text(A.ncid, NC_GLOBAL, "programVersion", 3, "1.0"), "Writing program version."))
        return 1;
    if (checkNCerr(nc_put_att_text(A.ncid, NC_GLOBAL, "Conventions", 12, "AMBERRESTART"), "Writing conventions."))
        return 1;
    if (checkNCerr(nc_put_att_text(A.ncid, NC_GLOBAL, "ConventionVersion", 3, "1.0"), "Writing conventions version."))
        return 1;
    ...
    if (restart_temp >= 0) {
        ...
        if (checkNCerr(nc_def_var(A.ncid, NCTEMPERATURE, NC_DOUBLE, 0, dimensionID, &(A.TempVID)),
                       "Defining replica temperature in netcdf restart."))
            return 1;
    ...
    // Set fill mode
    if (checkNCerr(nc_set_fill(A.ncid, NC_NOFILL, dimensionID), "NetCDF setting fill value.")) return 1;

    // End netcdf definitions
    if (checkNCerr(nc_enddef(A.ncid), "NetCDF error on ending definitions.")) return 1;
    ...
    if (checkNCerr(nc_put_vara_text(A.ncid, A.spatialVID, start, count, xyz),
                   "Error on NetCDF output of spatial VID 'x', 'y' and 'z'"))
        return 1;
    ...
    if (checkNCerr(nc_put_vara_text(A.ncid, A.cell_spatialVID, start, count, xyz),
                   "Error on NetCDF output of cell spatial VID 'a', 'b' and 'c'"))
        return 1;
    ...
    count[0] = A.ncatom;
    count[1] = 3;
    if (checkNCerr(nc_put_vara_double(A.ncid, A.coordVID, start, count, X), "Netcdf restart Writing coordinates"))
        return 1;

    // write velocity
    if (Velo != NULL) {
        if (checkNCerr(nc_put_vara_double(A.ncid, A.velocityVID, start, count, Velo),
                       "Netcdf restart writing velocity"))
            return 1;
    }
    ...
    if (box != NULL) {
        count[0] = 3;
        count[1] = 0;
        if (checkNCerr(nc_put_vara_double(A.ncid, A.cellLengthVID, start, count, box), "Writing cell lengths."))
            return 1;
        if (checkNCerr(nc_put_vara_double(A.ncid, A.cellAngleVID, start, count, box + 3), "Writing cell angles."))
            return 1;
    }
    ...
    if (restart_time >= 0)
        ...
        restartTime = 0;
    if (checkNCerr(nc_put_var_double(A.ncid, A.timeVID, &restartTime), "Writing restart time.")) return 1;

    // write temperature
    if (restart_temp >= 0) {
        if (checkNCerr(nc_put_var_double(A.ncid, A.TempVID, &restart_temp), "Writing restart temperature.")) return 1;
    }
...
int netcdfCreate(struct AmberNetcdf *A, const char *filename, int natom, int isBox) {
    ...
    // Create file
    if (checkNCerr(nc_create(filename, NC_64BIT_OFFSET, &(A->ncid)), "Creating Netcdf file %s", filename)) return 1;
    if (debug > 0) fprintf(stdout, "    Successfully created Netcdf file %s, ncid %i\n", filename, A->ncid);

    // Frame, Time
    if (checkNCerr(nc_def_dim(A->ncid, NCFRAME, NC_UNLIMITED, &(A->frameDID)), "Defining frame dimension.")) return 1;
    ...
    // Atoms
    if (checkNCerr(nc_def_dim(A->ncid, NCATOM, natom, &(A->atomDID)), "Defining atom dimension.")) return 1;
    A->ncatom = natom;
    A->ncatom3 = natom * 3;
    ...
    // Attributes
    if (checkNCerr(nc_put_att_text(A->ncid, NC_GLOBAL, "title", strlen(title), title), "Writing title.")) {
        if (title != NULL) free(title);
        return 1;
    }
    if (title != NULL) free(title);
    if (checkNCerr(nc_put_att_text(A->ncid, NC_GLOBAL, "application", 6, "Tinker"), "Writing application.")) return 1;
    if (checkNCerr(nc_put_att_text(A->ncid, NC_GLOBAL, "program", 6, "Tinker"), "Writing program.")) return 1;
    if (checkNCerr(nc_put_att_text(A->ncid, NC_GLOBAL, "programVersion", 3, "7.1"), "Writing program version."))
        return 1;
    if (checkNCerr(nc_put_att_text(A->ncid, NC_GLOBAL, "Conventions", 5, "AMBER"), "Writing conventions.")) return 1;
    if (checkNCerr(nc_put_att_text(A->ncid, NC_GLOBAL, "ConventionVersion", 3, "1.0"), "Writing conventions version."))
        return 1;
...
int netcdfGetVelocity(struct AmberNetcdf *A, int set, double *V) {
    ...
    if (A->isNCrestart) {
        ...
        if (A->velocityVID != -1) {
            ...
            count[0] = A->ncatom;
            count[1] = 3;
            if (checkNCerr(nc_get_vara_double(A->ncid, A->velocityVID, start, count, V), "Getting velocities") != 0)
                return 1;
        } else {
            printf("Warning: No velocities in restart, setting all to 0.0\n");
            for (i = 0; i < A->ncatom3; i++) V[i] = 0.0;
        }
        // ---------- Trajectory
    } else {
        if (A->velocityVID != -1) {
            ...
            if (checkNCerr(nc_get_vara_float(A->ncid, A->velocityVID, start, count, Velo),
                           "Getting velocities at frame %i", set) != 0) {
                free(Velo);
                return 1;
            }
            // Convert from single precision to double precision
            for (i = 0; i < A->ncatom3; i++) V[i] = (double)Velo[i];
            free(Velo);
        } else {
...
int netcdfGetFrame(struct AmberNetcdf *A, int set, double *X, double *box) {
    ...
    if (A->isNCrestart) {
        // Get temperature
        if (A->TempVID != -1) {
            if (checkNCerr(nc_get_var_double(A->ncid, A->TempVID, &(A->temp0)), "Getting restart temperature.") != 0)
                return 1;
        ...
        count[0] = A->ncatom;
        count[1] = 3;
        if (checkNCerr(nc_get_vara_double(A->ncid, A->coordVID, start, count, X), "Getting Coords") != 0) return 1;

        // Read box info
        if (box != NULL && A->cellLengthVID != -1) {
            count[0] = 3;
            count[1] = 0;
            if (checkNCerr(nc_get_vara_double(A->ncid, A->cellLengthVID, start, count, box), "Getting cell lengths.") !=
                0)
                return 1;
            if (checkNCerr(nc_get_vara_double(A->ncid, A->cellAngleVID, start, count, box + 3),
                           "Getting cell angles.") != 0)
                return 1;
        }
        ...
        count[1] = A->ncatom;
        count[2] = 3;
        if (checkNCerr(nc_get_vara_float(A->ncid, A->coordVID, start, count, Coord), "Getting frame %i", set) != 0) {
            free(Coord);
            return 1;
        }
        // Convert from single precision to double precision
        for (i = 0; i < A->ncatom3; i++) X[i] = (double)Coord[i];
        free(Coord);
        // Get temperature
        if (A->TempVID != -1) {
            count[0] = 1;
...
int netcdfWriteFrame(struct AmberNetcdf *A, int set, double *X, double *box) {
    ...
    // ---------- Restart
    if (A->isNCrestart) {
        fprintf(stderr, "Error: Called netcdfWriteFrame for netcdf restart; use netcdfWriteRestart instead.\n");
        return 1;
    }
    // ---------- Trajectory
    // Allocate space for single-precision coords
    ...
    for (i = 0; i < A->ncatom3; i++) Coord[i] = (float)X[i];
    // write coords
    start[0] = set;
    start[1] = 0;
    start[2] = 0;
    ...
    count[1] = A->ncatom;
    count[2] = 3;
    if (checkNCerr(nc_put_vara_float(A->ncid, A->coordVID, start, count, Coord), "Netcdf Writing frame %i", set))
        return 1;
    free(Coord);
    // write box
    if (box != NULL && A->cellLengthVID != -1) {
