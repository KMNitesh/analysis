...
T choose(T min, T max, const std::string &prompt, const Default<T> &defaultValue = {}, std::istream &in = std::cin,
...
template <typename T>
T choose(T min, const std::string &prompt, const Default<T> &defaultValue = {}, std::istream &in = std::cin,
         std::ostream &out = std::cout) {
    return choose<T>(min, std::numeric_limits<T>::max(), prompt, defaultValue, in, out);
}

bool choose_bool(const std::string &prompt, Default<bool> defaultValue = {}, std::istream &in = std::cin,
                 std::ostream &out = std::cout);
...
class range_object {
   ...
   public:
    ...
    bool operator!=(const range_object &) const { return _step > 0 ? _curr < _end : _curr > _end; }

    void operator++() { _curr += _step; }
...
class enumerate_iterator {
   ...
   public:
    ...
    bool operator!=(const enumerate_iterator<Iterable> &other) { return this->it != other.it; }
...
class PushIterable {
   ...
   public:
    ...
    bool operator!=(const PushIterable &) const { return _begin != _end || !queue.empty(); }
    ...
    void push_back(value_type value) { queue.push(value); }

    bool empty() const { return _begin == _end && queue.empty(); }
...
class combine_seq {
   ...
   public:
    ...
    combine_seq(const combine_seq &other) : _iter(other._iter), _curr(other._curr) {}

    bool operator!=(const combine_seq &) const { return !_iter.empty() || _curr.has_value(); }
