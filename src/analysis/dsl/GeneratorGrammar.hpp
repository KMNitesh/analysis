...
BOOST_FUSION_ADAPT_ADT(std::shared_ptr<Atom::residue_name_nums>,
                       (Atom::select_ranges, Atom::select_ranges, obj->val, /**/))

BOOST_FUSION_ADAPT_ADT(std::shared_ptr<Atom::molecule_nums>,
                       (std::vector<Atom::molecule_nums::Attr>, std::vector<Atom::molecule_nums::Attr>, obj->val, /**/))

BOOST_FUSION_ADAPT_ADT(std::shared_ptr<Atom::atom_name_nums>,
                       (Atom::select_ranges, Atom::select_ranges, obj->val, /**/))
...
struct GeneratorGrammar : karma::grammar<Iterator, Atom::Node()> {
    ...
    GeneratorGrammar() : GeneratorGrammar::base_type(start) {
        ...
        Operator =
            ...
            eps(at_c<0>(_val) == Atom::Op::AND) << (eps(_r1 > 2) << "(" | eps) << expr(2)[_1 = at_c<1>(_val)] << "&"
                                                << expr(2)[_1 = at_c<2>(_val)] << (eps(_r1 > 2) << ")" | eps) |
            eps(at_c<0>(_val) == Atom::Op::OR) << (eps(_r1 > 1) << "(" | eps) << expr(1)[_1 = at_c<1>(_val)] << "|"
