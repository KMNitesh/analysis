...
struct ForStmt {
    ForStmt(boost::any expr1, boost::any expr2, boost::any expr3, boost::any stmtBlock)
        : expr1(std::move(expr1)), expr2(std::move(expr2)), expr3(std::move(expr3)), stmt_block(std::move(stmtBlock)) {}
...
enum class LogicalOp { Equal, NotEqual, And, Or, LessEqual, Less, GreatEqual, Great, Not };
...
struct InterpreterGrammar : qi::grammar<Iterator, boost::any(), Skipper> {
    ...
    qi::rule<Iterator, boost::any(), Skipper> logical_or_expr, logical_and_expr, logical_equal_not_equal_expr,
        logical_comp_expr, arithmetic_low_expr, arithmetic_high_expr, prefix_unary_op_expr, literal, identifer,
        bitwise_or_expr, bitwise_and_expr, suffix_unary_op_expr;
    ...
    InterpreterGrammar() : InterpreterGrammar::base_type(languague) {
        ...
        identifer =
            as_string[lexeme[(alpha | char_("_")) >> *(alnum | char_("_"))]][_val = construct<Identifer>(_1, _pass)];
        ...
        function_call_expr =
            parenthese_expr[_val = _1] |
            identifer[_val = _1] >> *('(' > function_call_arguments_expr[_val = construct<Function>(_val, _1)] >> ')');
        ...
        prefix_unary_op_expr =
            '!' >> prefix_unary_op_expr[_val = construct<LogicalOperation>(LogicalOp::Not, _1)] |
            "++" >> prefix_unary_op_expr[_val = construct<ArithmeticOperation>(ArithmeticOp::PreIncrement, _1)] |
            "--" >> prefix_unary_op_expr[_val = construct<ArithmeticOperation>(ArithmeticOp::PreDecrement, _1)] |
            '+' >> prefix_unary_op_expr[_val = _1] |
            '-' >> prefix_unary_op_expr[_val = construct<ArithmeticOperation>(ArithmeticOp::Minus, _1)] |
            suffix_unary_op_expr[_val = _1];

        arithmetic_high_expr =
            prefix_unary_op_expr[_val = _1] >>
            *('*' >> prefix_unary_op_expr[_val = construct<ArithmeticOperation>(ArithmeticOp::Multiply, _val, _1)] |
              '/' >> prefix_unary_op_expr[_val = construct<ArithmeticOperation>(ArithmeticOp::Divide, _val, _1)] |
              '%' >> prefix_unary_op_expr[_val = construct<ArithmeticOperation>(ArithmeticOp::Mod, _val, _1)]);

        arithmetic_low_expr =
            arithmetic_high_expr[_val = _1] >>
            *('+' >> arithmetic_high_expr[_val = construct<ArithmeticOperation>(ArithmeticOp::Plus, _val, _1)] |
              '-' >> arithmetic_high_expr[_val = construct<ArithmeticOperation>(ArithmeticOp::Subtract, _val, _1)]);

        logical_comp_expr =
            arithmetic_low_expr[_val = _1] >>
        ...
        logical_equal_not_equal_expr =
            logical_comp_expr[_val = _1] >>
        ...
        bitwise_and_expr =
            logical_equal_not_equal_expr[_val = _1] >>
        ...
        assign_expr =
            ...
            -('=' >> assign_expr[_val = construct<AssignStmt>(_val, _1)] |
              ...
              "-=" >> assign_expr[_val = construct<AssignStmt>(
                                      ...
                                      AssignStmt::TYPE::Compound)] |
              "&=" >>
                  assign_expr[_val = construct<AssignStmt>(_val, construct<BitwiseOperation>(BitwiseOp::And, _val, _1),
                                                           AssignStmt::TYPE::Compound)] |
              "|=" >>
                  assign_expr[_val = construct<AssignStmt>(_val, construct<BitwiseOperation>(BitwiseOp::Or, _val, _1),
                                                           AssignStmt::TYPE::Compound)]);

        for_stmt = distinct(char_("a-zA-Z_0-9"))["for"] > '(' > assign_expr[_a = _1] > ';' > assign_expr[_b = _1] >
                   ';' > assign_expr[_c = _1] > ')' > '{' > stmts[_d = _1] > '}' >
                   eps[_val = construct<ForStmt>(_a, _b, _c, _d)];

        if_else_stmt = distinct(char_("a-zA-Z_0-9"))["if"] > '(' > assign_expr[_a = _1] > ')' > '{' > stmts[_b = _1] >
                       '}' > -(lit("else") > '{' > stmts[_c = _1] > '}') >
                       eps[_val = construct<IfElseStmt>(_a, _b, _c)];

        while_stmt = distinct(char_("a-zA-Z_0-9"))["while"] > '(' > assign_expr[_a = _1] > ')' > '{' > stmts[_b = _1] >
                     '}' > eps[_val = construct<WhileStmt>(_a, _b)];

        do_while_until_stmt =
            distinct(char_("a-zA-Z_0-9"))["do"] > '{' > stmts[_a = _1] > '}' >
            ((lit("until") > '(' > assign_expr[_b = _1] > ')' > ';' > eps[_val = construct<DoUntilStmt>(_a, _b)]) |
             (lit("while") > '(' > assign_expr[_b = _1] > ')' > ';' > eps[_val = construct<DoWhileStmt>(_a, _b)]));
        ...
        stmt %= break_stmt | continue_stmt | for_stmt | if_else_stmt | while_stmt | do_while_until_stmt |
                (assign_expr > ';');
        ...
        qi::on_error<qi::fail>(languague, std::cout << val("Error! Expecting") << _4 << val(" here: \"")
                                                    << phoenix::construct<std::string>(_3, _2) << val("\"")
                                                    << std::endl);
    }
};
...
public:
    using function_sig =
        std::function<boost::any(std::vector<std::tuple<std::string, std::function<bool(const boost::any)>,
                                                        std::function<std::vector<std::string>()>, boost::any>> &)>;

    FunctionObject() = default;

    FunctionObject(Interpreter *iterpreter, std::string name, function_sig f)
...
public:
    ...
    boost::any execute(boost::any ast) { return evalRightValue(std::move(ast)); }

    std::unordered_map<std::string, boost::any> &getVariables() { return variables; }

    FunctionObject &registerFunction(const std::string &name, const FunctionObject::function_sig &f) {
