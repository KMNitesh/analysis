...
c
      ...
      character*120 fname1, netcdfname1
      character*120 fname2, netcdfname2
...
c
      ...
      call getxyz
      usenetcdf1 = .false.
      call getnetcdf(netcdffile,usenetcdf1)
      call mechanic
c
...
c
      call getxyz
      usenetcdf2 = .false.
      call getnetcdf(netcdffile,usenetcdf2)
      call mechanic
      silent = .true.
c
c     set beginning and ending frame and the step increment
...
c
      ...
      rewind (unit=ixyz)
      if (usenetcdf1) then
        call opennetcdf(netcdfname1,inetcdf)
      end if
...
c
      ...
      rewind (unit=ixyz)
      if (usenetcdf2) then
        call opennetcdf(netcdfname2,inetcdf)
      end if
      do i = 1, start2-1
         if (usenetcdf2) then
            if (n .eq. 0) then
