...
c
      if (gmxtrr .and. isdynamic) then
          trrfilename = filename(1:leng)//'.trr'
          if (use_bounds) then
             call append_frame_x_box(trrfilename, leng+4, istep,
     &            istep*dt,xbox,ybox,zbox,alpha,beta,gamma,n,x,y,z)
          else
             call append_frame_x(trrfilename,leng+4,istep,
     &            istep*dt,n,x,y,z)
          end if
      else if (netcdf .and. isdynamic) then
          netcdffilename = filename(1:leng)//'.nc'
          if (use_bounds) then
             call netcdf_append_box_x(netcdffilename,leng+3,
     &            xbox,ybox,zbox,alpha,beta,gamma,n,x,y,z)
          else
             call netcdf_append_x(netcdffilename,leng+3,
     &            n,x,y,z)
          end if
      else
          ixyz = freeunit ()
  ...
  170 format (' Frame Number',13x,i10)
      if (gmxtrr .and. isdynamic) then
           write (iout,180)  trrfilename(1:trimtext(trrfilename))
      else if (netcdf .and. isdynamic) then
           write (iout,180)  netcdffilename(1:trimtext(netcdffilename))
      else
