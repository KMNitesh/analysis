...
static int GetDimInfo(int ncid, const char *attribute, int *length) {
    ...
    *length = 0;
    ...
    if (checkNCerr(nc_inq_dimid(ncid, attribute, &dimID),
                   "ncGetDimInfo: Getting dimID for attribute %s", attribute) != 0)
        return -1;

    // get Dim length
    if (checkNCerr(nc_inq_dimlen(ncid, dimID, &slength),
                   "ncGetDimInfo: Getting length for attribute %s", attribute) != 0)
        return -1;
...
int netcdfWriteRestart(char *filename, int natom, double *X, double *Velo,
    ...
    if (checkNCerr(nc_create(filename, NC_64BIT_OFFSET, &(A.ncid)),
                   "Creating Netcdf restart file %s", filename))
        return 1;
    //if (debug>0)
    ...
    if (checkNCerr(nc_put_vara_double(A.ncid, A.coordVID, start, count, X),
                   "Netcdf restart Writing coordinates"))
        return 1;
...
int netcdfCreate(struct AmberNetcdf *A, const char *filename, int natom, int isBox) {
    ...
    // Create file
    if (checkNCerr(nc_create(filename, NC_64BIT_OFFSET, &(A->ncid)),
                   "Creating Netcdf file %s", filename))
        return 1;
    if (debug > 0)
    ...
    if (title != NULL) free(title);
    if (checkNCerr(nc_put_att_text(A->ncid, NC_GLOBAL, "application", 6, "Tinker"),
                   "Writing application."))
        return 1;
    if (checkNCerr(nc_put_att_text(A->ncid, NC_GLOBAL, "program", 6, "Tinker"),
                   "Writing program."))
        return 1;
    if (checkNCerr(nc_put_att_text(A->ncid, NC_GLOBAL, "programVersion", 3, "7.1"),
                   "Writing program version."))
        return 1;
