...
void GROWriter::write(const std::string &filename, std::shared_ptr <Frame> &frame) {
    std::fstream f;
    f.open(filename, std::ios::out);

    f << frame->title << ", Generated by Analysis (Miaoren) , t = 0.0" << std::endl;

    f << frame->atom_list.size() << std::endl;
    int mol_index = 1;
