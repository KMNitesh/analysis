...
c
      ...
      integer*8 itrr, inetcdf
      logical usetrr, usenetcdf
      character*120 trrfile, netcdffile
...
c
      if (numj.ne.0 .and. numk.ne.0) then
         ...
         do i = 1, nbin
            ...
            if (i.ne.1) then
                integral(i) = dble(hist(i))/dble(nframe) + integral(i-1)
            else
                integral(i) = dble(hist(i))/dble(nframe)
            end if
...
c
  ...
  250 format (/,5x,'Bin',9x,'Counts',7x,'Distance',7x,'Raw g(r)',
     &           4x,'Smooth g(r)',7x,"Integral"/)
      do i = 1, nbin
         write (iout,260)  i,hist(i),(dble(i)-0.5d0)*width,gr(i),gs(i),
     &                       integral(i)
  260    format (i8,i15,3x,f12.4,3x,f12.4,3x,f12.4,f12.4)
      end do
c
